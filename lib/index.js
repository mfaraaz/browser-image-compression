 ...
 */
async function imageCompression(files, options) {
  const opts = { ...options };
  ...
  const useWebWorker =
    typeof opts.useWebWorker === "boolean" ? opts.useWebWorker : true;
  ...
  if (useWebWorker && typeof Worker === "function" && !inWebWorker) {
    try {
      ...
      compressedFile = await Promise.all(
        files.map((file) => compressOnWebWorker(file, opts))
      ).then((res) => res);
    } catch (e) {
      if (process.env.BUILD === "development") {
        console.warn(
          "Run compression in web worker failed:",
          e,
          ", fall back to main thread"
        );
      }
      compressedFile = await Promise.all(
        files.map((file) => compress(file, opts))
      ).then((res) => res);
    }
  } else {
    compressedFile = await Promise.all(
